...
final class AppBehaviorTests: XCTestCase {
    ...
    func testOfflineFallbackPolicyRequiresExplicitOptIn() {
        XCTAssertEqual(
            OfflineFallbackPolicy.route(
                selectedProvider: .groq,
                fallbackEnabled: false,
                connectivity: .offline,
                appleSpeechReady: true
            ),
            .selectedProvider
        )
        XCTAssertEqual(
    ...
    func testOfflineFallbackPolicyNeverOverridesExplicitAppleSelection() {
        XCTAssertEqual(
            OfflineFallbackPolicy.route(
    ...
    func testOnboardingIncludesOptionalOfflineModeBeforeScratchpad() {
        XCTAssertEqual(
            ...
            [.welcome, .permissions, .provider, .offlineMode, .ready]
        )
    ...
    func testOfflineFallbackSetupPersistsOnlyAfterExplicitConfiguration() async throws {
        ...
        let model = AppModel(
            ...
            appleSpeechProvider: DelayedAppleSpeechProvider(),
            defaults: defaults,
    ...
    func testFailedOfflineFallbackSetupRemainsDisabled() async throws {
        let suiteName = "ai.dictator.tests.offline.failure.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        defaults.set(true, forKey: "onboardingComplete")
        defaults.set(ProviderKind.groq.rawValue, forKey: "selectedSTT")
        let model = AppModel(
            keychain: EmptyCredentialStore(),
            appleSpeechProvider: UnavailableAppleSpeechProvider(),
            defaults: defaults,
            connectivity: StaticConnectivityMonitor(state: .online)
        )
    ...
    func testChangingOfflineLanguageRequiresSetupAgain() async throws {
        let suiteName = "ai.dictator.tests.offline.locale.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        defaults.set(true, forKey: "onboardingComplete")
        defaults.set(ProviderKind.groq.rawValue, forKey: "selectedSTT")
        defaults.set(true, forKey: "offlineFallbackEnabled")
        let model = AppModel(
            keychain: EmptyCredentialStore(),
            appleSpeechProvider: DelayedAppleSpeechProvider(),
            defaults: defaults,
