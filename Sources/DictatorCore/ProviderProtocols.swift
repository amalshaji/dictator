...
public protocol SpeechToTextProvider: Sendable {
    var metadata: STTProviderMetadata { get }
...
public struct CleanupPrompt: Sendable {
    public static let currentVersion = "cleanup-v1"
    ...
    public static func system(vocabulary: [VocabularyEntry], styleInstruction: String? = nil) -> String {
        ...
        let styleRule = styleInstruction?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
            ? "\nWriting style: \(styleInstruction!). Apply only presentation changes; never change meaning."
            : ""
